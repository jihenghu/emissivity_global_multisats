subroutine Write_TB_Cloud_HDF5(filename,nchannel,npixel,nscan, &
	lon,lat,stime,TB,lwp,iwp,cth,LZA,cfr,clear,lsm,flag,missrate,status)

      USE HDF5   
      IMPLICIT NONE
      CHARACTER(*), INTENT(IN) :: filename	  
	  Integer, INTENT(IN) :: nscan,npixel,nchannel
      REAL, DIMENSION(npixel,nscan), INTENT(IN) :: lon, lat  
      ! REAL, DIMENSION(nscan), INTENT(IN) 	  :: stime	  
      REAL, DIMENSION(npixel,nscan), INTENT(IN) 	  :: stime	  
	  REAL, DIMENSION(nchannel,npixel,nscan), INTENT(IN) :: TB
	  
	  REAL, DIMENSION(3,npixel,nscan), INTENT(IN) :: lwp,iwp
	  REAL, DIMENSION(3,npixel,nscan), INTENT(IN) :: cth,cfr,clear
  ...
  ! =====================================================================
      ...
      dsetname="Liquid_Water_Path"		  
	  CALL h5dcreate_f(file_id, trim(adjustl(dsetname)), H5T_NATIVE_REAL, dspace_id,dset_id, status)
      CALL h5dwrite_f(dset_id, H5T_NATIVE_REAL, lwp, data_dims4, status)
	  	  CALL h5screate_simple_f(arank, adims, aspace_id, status)
		  CALL h5tcopy_f(H5T_NATIVE_CHARACTER, atype_id, status)
		  ...
		  CALL h5tset_size_f(atype_id, attrlen, status)
		  CALL h5acreate_f(dset_id, "units", atype_id, aspace_id, attr_id, status)
		  CALL h5awrite_f(attr_id, atype_id, "g/m2", adims, status)
		  CALL h5aclose_f(attr_id, status)

		  attrlen=35
		  CALL h5tset_size_f(atype_id, attrlen, status)		  
		  CALL h5acreate_f(dset_id, "long_name", atype_id, aspace_id, attr_id, status)
		  CALL h5awrite_f(attr_id, atype_id, "Cloud Liquid Water Path in MW Beams", adims, status)
		  CALL h5aclose_f(attr_id, status)
      ...
      dsetname="Ice_Water_Path"		  
	  CALL h5dcreate_f(file_id, trim(adjustl(dsetname)), H5T_NATIVE_REAL, dspace_id,dset_id, status)
      CALL h5dwrite_f(dset_id, H5T_NATIVE_REAL, iwp, data_dims4, status)
	  	  CALL h5screate_simple_f(arank, adims, aspace_id, status)
		  CALL h5tcopy_f(H5T_NATIVE_CHARACTER, atype_id, status)
		  ...
		  CALL h5tset_size_f(atype_id, attrlen, status)		  
		  CALL h5acreate_f(dset_id, "long_name", atype_id, aspace_id, attr_id, status)
		  CALL h5awrite_f(attr_id, atype_id, "Cloud Ice Water Path in MW Beams", adims, status)
		  CALL h5aclose_f(attr_id, status)
      ...
      dsetname="Cloud_Top_Height"		  
	  CALL h5dcreate_f(file_id, trim(adjustl(dsetname)), H5T_NATIVE_REAL, dspace_id,dset_id, status)
      CALL h5dwrite_f(dset_id, H5T_NATIVE_REAL, cth, data_dims4, status)
	  	  CALL h5screate_simple_f(arank, adims, aspace_id, status)
		  CALL h5tcopy_f(H5T_NATIVE_CHARACTER, atype_id, status)
		  ...
		  CALL h5tset_size_f(atype_id, attrlen, status)
		  CALL h5acreate_f(dset_id, "units", atype_id, aspace_id, attr_id, status)
		  CALL h5awrite_f(attr_id, atype_id, "km", adims, status)
		  CALL h5aclose_f(attr_id, status)

		  attrlen=28
		  CALL h5tset_size_f(atype_id, attrlen, status)		  
		  CALL h5acreate_f(dset_id, "long_name", atype_id, aspace_id, attr_id, status)
		  CALL h5awrite_f(attr_id, atype_id, "Cloud Top Height in MW Beams", adims, status)
		  CALL h5aclose_f(attr_id, status)
      ...
      dsetname="Latitude"	  	  
      CALL h5dcreate_f(file_id, trim(adjustl(dsetname)), H5T_NATIVE_REAL, dspace_id,dset_id, status)
      CALL h5dwrite_f(dset_id, H5T_NATIVE_REAL, lat, data_dims2, status)
	  	  CALL h5screate_simple_f(arank, adims, aspace_id, status)
		  CALL h5tcopy_f(H5T_NATIVE_CHARACTER, atype_id, status)
		  CALL h5tset_size_f(atype_id, attrlen, status)
		  attrlen=12
      ...
      dsetname="Longitude"	  	  
      CALL h5dcreate_f(file_id, trim(adjustl(dsetname)), H5T_NATIVE_REAL, dspace_id,dset_id, status)
      CALL h5dwrite_f(dset_id, H5T_NATIVE_REAL, lon, data_dims2, status)
	  	  CALL h5screate_simple_f(arank, adims, aspace_id, status)
		  CALL h5tcopy_f(H5T_NATIVE_CHARACTER, atype_id, status)
	  ...
	  ! rank = 1	  	  
      ! CALL h5screate_simple_f(rank, data_dims1, dspace_id, status)
      ! dsetname="Scan_Time_UTC"	  	  
      ! CALL h5dcreate_f(file_id, trim(adjustl(dsetname)), H5T_NATIVE_REAL, dspace_id,dset_id, status)
      ! CALL h5dwrite_f(dset_id, H5T_NATIVE_REAL, stime, data_dims1, status)
...
end subroutine Write_TB_Cloud_HDF5